...
int main(int argc, char **argv) {
  // I pedantically check that all the registers allocate properly, if they aren't there's something very wrong
  // one way or another, so I think it's a suitable action to immediately cease execution if this occurs, but it
  // really shouldn't ever happen, I hope. At the very least, I can say it won't try dereferencing a null pointer (I guess).

  // REGISTERS
  short *const REGS = calloc(9, sizeof(unsigned));
  ...
  */
  ...
  // Read code into CS
  if(argc != 2) {
    printf("Takes exactly 1 argument, got %d.", argc - 1);
    return -1;
  }
